...
<div class="container-fluid px-3 px-lg-4 py-3">
        ...
        <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
            <div>
                <div class="d-flex align-items-center gap-2">
                    <i class="bi bi-envelope-gear-fill fs-4 text-primary"></i>
                        ...
                        @forelse ($mailSettings as $setting)
                            @php
                                $payload = [
                                    ...
                                    'update_url' => route('dashboard.mail_settings.update', $setting),
                                    'delete_url' => route('dashboard.mail_settings.destroy', $setting),
                                ];
                            @endphp
                            <tr>
                                <td>
                                ...
                                <td>
                                    <div class="d-flex justify-content-end gap-2">
                                        @unless ($setting->is_active)
                                            <form method="POST" action="{{ route('dashboard.mail_settings.activate', $setting) }}">
                                                @csrf
                                                @method('PATCH')
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="text-center py-5 text-muted">
                                    <i class="bi bi-envelope-gear d-block fs-3 mb-2"></i>
    ...
    <div class="modal fade" id="createMailSettingModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content">
                <form method="POST" action="{{ route('dashboard.mail_settings.store') }}">
                    @csrf
                    ...
                    <div class="modal-body">
                        @include('dashboard.mail-settings.partials.fields', ['mode' => 'create'])
                    </div>
    ...
    <div class="modal fade" id="editMailSettingModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content">
                <form method="POST" action="{{ route('dashboard.mail_settings.content') }}" id="editMailSettingForm">
                    @csrf
                    @method('PUT')
                    <div class="modal-header">
                    ...
                    <div class="modal-body">
                        @include('dashboard.mail-settings.partials.fields', ['mode' => 'edit'])
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
                        <button type="submit" class="btn btn-primary">
    ...
    <div class="modal fade" id="deleteMailSettingModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <form method="POST" action="{{ route('dashboard.mail_settings.content') }}" id="deleteMailSettingForm">
                    @csrf
                    @method('DELETE')
                    <div class="modal-header">
...
@section('scripts')
    <script src="{{ asset('js/adminMailSettings.js') }}"></script>
@endsection
